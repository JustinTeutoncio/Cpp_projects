...
//Insert vs Merge vs Heap sorting
#include<iostream>
#include<time.h>
using namespace std;
...
void beginprogram() {
	int a[6] = { 100, 1000, 10000, 50000, 100000, 200000 };//input sizes
	cout << "This program will compare the sort times of Insertion Sort, Merge Sort, and Heap Sort.\nThe time is measured in seconds.\n";
	time_t t1, t2, t3, t4;//create time variables
	cout << "Input size    Insert   Merge   Heap" << endl;
	for (int i = 0; i < 6; i++){
		int *b = new int[a[i]];//will hold the array
		int *b2 = new int[a[i]];//will be the copy 
		int *b3 = new int[a[i]];//additonal copy
		int temp = 0;
		for (int j = 0; j < a[i]; j++){
...
int main() {
	srand(time(NULL));//seeds randomness at start based on system clock
